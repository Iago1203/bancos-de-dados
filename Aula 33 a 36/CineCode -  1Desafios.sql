...
select * from filmes where genero like  'Mistério, Suspense%' or 'Ficção, Fantasia%';
...
select titulo, preco_alugel from filmes order by preco_alugel desc limit 5;
...
update clientes set estado = 'Rio de Janeiro' where id_cliente = 109;
...
-- Qual é o erro? _Sem o =_________________________________________________________
-- Correção:
SELECT titulo, genero, ano_lancamento FROM filmes WHERE ano_lancamento = 2005 ORDER BY titulo;
...
-- Qual é o erro? _Região estava como Estado_________________________________________________________
-- Correção:
...
-- Qual é o erro? _Sem o is e em ''_________________________________________________________
-- Correção:
...
UPDATE filmes set genero = 'Fantasia' where titulo like 'Harry Poter%';
